...
static int CheckIfAllKeysKnown(TMPQArchive * ha, const char** listFileEntries, DWORD dwEntryCount, LPDWORD pFileKeys)
{
    TFileEntry * pFileTableEnd = ha->pFileTable + ha->dwFileTableSize;
    TFileEntry * pFileEntry;
    DWORD dwBlockIndex = 0;
    ...
    if(listFileEntries != NULL && dwEntryCount > 0)
    {
        ...
        nError = SFileAddListFileEntries((HANDLE)ha, listFileEntries, dwEntryCount);
    }
...
bool WINAPI SFileCompactWithList(HANDLE hMpq, const char** listFileEntries, DWORD dwEntryCount)
{
    ...
    if(nError == ERROR_SUCCESS)
    {
        ...
        FileStream_GetSize(ha->pStream, &(ha->CompactTotalBytes));
        ha->CompactBytesProcessed = 0;
        nError = CheckIfAllKeysKnown(ha, listFileEntries, dwEntryCount, pFileKeys);
    }
...
bool WINAPI SFileCompactWithAddition(HANDLE hMpq, const char* listFileAddition)
{
    const char** listFileEntries = &listFileAddition;
    return SFileCompactWithList(hMpq, listFileEntries, 1);
}
