    ...
    DWORD dwEntryCount)
{
    ...
    return (listFileEntries != NULL && dwEntryCount > 0) ? ERROR_SUCCESS : ERROR_FILE_CORRUPT;
}
...
int WINAPI SFileAddListFileEntries(HANDLE hMpq, const char** listFileEntries, DWORD dwEntryCount)
{
    TMPQArchive * ha = (TMPQArchive *)hMpq;
    int nError = ERROR_SUCCESS;
    ...
    {
        if(listFileEntries != NULL && dwEntryCount > 0)
            nError = SFileAddArbitraryListFile(ha, listFileEntries, dwEntryCount);
        else
            nError = SFileAddInternalListFile(ha, hMpq);
