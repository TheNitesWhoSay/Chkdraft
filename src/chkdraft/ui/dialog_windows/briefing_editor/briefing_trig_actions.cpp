    ...
    if ( action.actionType < Chk::Action::NumBriefingActionTypes )
    {
        ...
        else
        {
            ...
            if ( suggestion.data )
            {
                ...
                {
                    case Chk::Action::ArgField::LocationId: if ( action.locationId != suggestion.data.value() ) editAction.locationId = suggestion.data.value(); break; // u32
                    case Chk::Action::ArgField::StringId: if ( action.stringId != suggestion.data.value() ) editAction.stringId = suggestion.data.value(); break; // u32
                    case Chk::Action::ArgField::SoundStringId: if ( action.soundStringId != suggestion.data.value() ) editAction.soundStringId = suggestion.data.value(); break; // u32
                    case Chk::Action::ArgField::Time: if ( action.time != suggestion.data.value() ) editAction.time = suggestion.data.value(); break; // u32
                    case Chk::Action::ArgField::Group: if ( action.group != suggestion.data.value() ) editAction.group = suggestion.data.value(); break; // u32
                    case Chk::Action::ArgField::Number: if ( action.number != suggestion.data.value() ) editAction.number = suggestion.data.value(); break; // u32
                    case Chk::Action::ArgField::Type: if ( action.type != suggestion.data.value() ) editAction.type = u16(suggestion.data.value()); break; // u16
                    case Chk::Action::ArgField::ActionType: if ( action.actionType != suggestion.data.value() ) editAction.actionType = Chk::Action::Type(suggestion.data.value()); break; // u8
                    case Chk::Action::ArgField::Type2: if ( action.type2 != suggestion.data.value() ) editAction.type2 = u8(suggestion.data.value()); break; // u8
                    case Chk::Action::ArgField::Flags: if ( action.flags != suggestion.data.value() ) editAction.flags = u8(suggestion.data.value()); break; // u8
                    case Chk::Action::ArgField::Padding: if ( action.padding != suggestion.data.value() ) editAction.padding = u8(suggestion.data.value()); break; // u8
                    case Chk::Action::ArgField::MaskFlag: if ( action.maskFlag != suggestion.data.value() ) editAction.maskFlag = Chk::Action::MaskFlag(suggestion.data.value()); break; // u16
