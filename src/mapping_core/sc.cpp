    ...
    std::array<u16, Sprite::TotalSprites> & doodadSpriteFlags, std::array<u16, Unit::TotalTypes> & doodadUnitFlags)
{
    ...
    remap[0].load(archiveCluster, ofireFilePath);
    remap[1].load(archiveCluster, gfireFilePath);
    remap[2].load(archiveCluster, bfireFilePath);
    remap[3].load(archiveCluster, bexplFilePath);
    remap[4].load(archiveCluster, trans50FilePath);
    remap[5].load(archiveCluster, redFilePath);
    remap[6].load(archiveCluster, greenFilePath);
    dark.load(archiveCluster, darkFilePath);
    shift.load(archiveCluster, shiftFilePath);
...
bool Sc::IScripts::load(ArchiveCluster & archiveCluster)
{
    auto iscriptBin = Sc::Data::GetAsset(archiveCluster, "scripts\\iscript.bin");
    if ( !iscriptBin )
    {
        logger.error() << "Failed to load scripts\\iscript.bin" << std::endl;
        return false;
