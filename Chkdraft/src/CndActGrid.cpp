...
bool CndActGrid::allowKeyNavDuringEdit(GVKey key)
{
	switch ( key )
	{
		case GVUp:
...
void CndActGrid::EditTextChanged(std::string &str)
{
	ignoreChange = true;
...
BOOL CndActGrid::ProcessTrack(NMHEADER* nmHeader)
{
	if ( nmHeader->iItem == 0 )
