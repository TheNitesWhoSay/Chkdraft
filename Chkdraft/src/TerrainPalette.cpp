...
void TerrainPaletteWindow::DoPaint()
{
    if ( CM != nullptr && StartBufferedPaint() != NULL )
    {
        ...
        TileSet* tileset = &chkd.scData.tilesets.set[CM->getTileset()];
            ...
            for ( s32 column = 0; column < TILES_PER_ROW; column++ )
            {
                ...
                else
                {
                    ...
                    if ( CM->DisplayingTileNums() )
                    {
                        ...
                        ExtTextOut(paintDc, column*PIXELS_PER_TILE + 3, row*PIXELS_PER_TILE - yOffset + 2, ETO_OPAQUE, &nullRect, TileHex, std::strlen(TileHex), 0);
